<?php 
        ...
        if (!isset($_SESSION["word"]) ||!isset($_GET["submit"])){
           ...
           if (isset($_GET["letterInput"]) && strlen($_GET["letterInput"])== 1){
            for ($i = 0; $i < $_SESSION["word"]; $i++){
                if ($_SESSION["word"][$i] = $_GET["letterInput"]){
                    $_SESSION["wordToShow"][$i] = $_GET["letterInput"];
                } 
            }
           }else{
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-center mb-4">
                            ...
                            <?php 
                                for ($i=0;$i < strlen($_SESSION["wordToShow"]);$i++){
                                    echo $_SESSION["wordToShow"][$i] . " ";
                                }
                            ?>
                           </p> 
                        </div>
                        <form id="guessForm">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="letterInput" id="letterInput" maxlength="1" required>
                            </div>
                            <div class="d-grid">
                                <button type="submit" value="" id="submit" name="submit" class="btn btn-primary">Adivinar Letra</button>
                            </div>
                        </form>
